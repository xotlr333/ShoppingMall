...
<%
 ...
 msg="select * from d_animal where d_code="+ data ;
 ST=CN.createStatement( );
 RS=ST.executeQuery(msg);
 if(RS.next()==true){
	code=RS.getInt("d_code"); 
	id=RS.getString("d_id");
	title=RS.getString("d_title");
	content=RS.getString("d_content");
	animal_name=RS.getString("d_animal_name");
	gender=RS.getString("d_gender");
	file=RS.getString("d_file_name");
 }
%>

<br>
<form name="w_form" method="post" enctype="multipart/form-data" action="animalEditSave_dog.jsp" >
 <div class="container">
     <table width=860px>
		   
		   <tr>
       <td><b><%=id%></b>님의 글 입니다<p><input type="hidden" name="d_code" value="<%=code %>"></td>
       </tr>
       
       <tr align="right">
       <td colspan="4">
        <a href="animalList_dog.jsp"><input type="button" value="목록" style="background-color:white ; color : #fdd017; width:100px; height:38px;"></a>
     ...
     <h4>게시판 글 수정하기</h4><p>
       <div>
        <input type="text" placeholder="제목을 입력해주세요." name="d_title" value="<%=title%>" >
       </div><br>
         
	     <div>
	       <textarea style="resize: none; " name="d_content" cols="95" rows="10" class="textarea"  placeholder="내용을 입력해주세요." ><%=content%></textarea>
	     </div>
     ...
     <h4>반려동물 정보</h4>
        <div>   
          <table width="860px">
            ...
            <td>
             <label>이름&nbsp;</label>
                <input type="text" placeholder="반려동물 이름" name="d_animal_name" value="<%=animal_name%>"> 
            </td>
              
            <td style="text-align:left;">
              <label>성별&nbsp;&nbsp;</label>
               <input type="radio" id="gender_m" name="d_gender" value="male" style="padding-right:20px;" <%if(gender.equals("male")){%>checked<%}%>>
               <label for="gender_m">수컷</label>&nbsp;
               <input type="radio" id="gender_w" name="d_gender" value="female" <%if(gender.equals("female")){%>checked<%}%>>
